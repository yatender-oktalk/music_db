...
result = Repo.insert_all("artists", [[name: "John Coltrane"]])
# => {1, nil}

assert {1, nil} = result

if Repo.using_postgres?() do
  result = Repo.insert_all("artists", [[name: "John Coltrane"]], returning: [:id])
  # => {1, [%{id: 8}]}

  assert {1, [%{id: _id}]} = result
end

result = Repo.insert(%Artist{name: "John Coltrane"})
